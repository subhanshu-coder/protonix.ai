...
const userSchema = new mongoose.Schema({
  fullName: { type: String, required: function () { return !this.googleId; }, trim: true },
...
// ── EMAIL — Nodemailer + Gmail (any email, no domain needed) ──
...
const sendEmail = async ({ to, subject, html }) => {
  return transporter.sendMail({ from: `"Protonix.AI" <${process.env.EMAIL_FROM}>`, to, subject, html });
};
(async () => {
  try { await transporter.verify(); console.log('✅ Email (Nodemailer+Gmail) ready'); }
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(apiUrl, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${apiKey.trim()}`, 'HTTP-Referer': process.env.CLIENT_URL || '', 'X-Title': 'Protonix AI', 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: modelPath, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: message }], max_tokens: 800, temperature }),
    });
...
app.post('/api/auth/google', async (req, res) => {
  try {
    ...
    if (!user) user = await User.create({ fullName: name, email, googleId, avatar: picture, provider: 'google', isVerified: true });
    sendTokenResponse(user, 200, res);
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;background:#0f0f1a;color:#fff;border-radius:14px;overflow:hidden;">
          <div style="padding:28px 32px;background:linear-gradient(135deg,#0d1117,#1a1f35);border-bottom:1px solid rgba(255,255,255,0.07);text-align:center;">
            <h1 style="margin:0;font-size:1.6rem;font-weight:800;">PROTONIX<span style="color:#00e5ff;">.AI</span></h1>
            <p style="margin:4px 0 0;color:rgba(255,255,255,0.35);font-size:0.78rem;">All AIs, One Tab</p>
          </div>
          <div style="padding:36px 32px;">
            <h2 style="margin:0 0 10px;font-size:1.2rem;">Reset Your Password</h2>
            ...
            <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:8px;padding:14px 16px;">
              <p style="margin:0;color:rgba(255,255,255,0.38);font-size:0.78rem;line-height:1.7;">
            ...
            <p style="margin-top:20px;color:rgba(255,255,255,0.22);font-size:0.72rem;word-break:break-all;">
              Link not working? Copy into browser:<br/><span style="color:#00e5ff;">${resetUrl}</span>
            </p>
          </div>
...
// ── TEST EMAIL — https://protonix-ai.onrender.com/api/test-email?to=anyone@gmail.com ──
app.get('/api/test-email', async (req, res) => {
  const to = req.query.to || process.env.EMAIL_FROM;
  try {
    await sendEmail({ to, subject: 'Protonix.AI — Email Test ✅', html: `<div style="font-family:Arial;padding:24px;background:#0f0f1a;color:#fff;border-radius:10px;"><h2 style="color:#00e5ff;">Email working! ✅</h2><p>Nodemailer + Gmail is working. Sent to: ${to}</p></div>` });
